...
export async function searchYouTubeMusic(
  ...
  /*
   * IMPORTANT:
   * Only request videos that can be embedded and played
   * outside youtube.com.
   */
  ...
  try {
    ...
    /*
     * Verify embeddability from videos.list as an additional
     * safety check before creating playable Track objects.
     */
    ...
    if (videoIds) {
      try {
        ...
        const detailsRes = await fetch(
          `/api/youtube?${detailsParams.toString()}`,
          { headers }
        );
        ...
        if (detailsRes.ok) {
          ...
          (detailsData.items || []).forEach((vid: any) => {
            ...
            /*
             * status.embeddable === true means the video owner
             * allows embedding.
             */
            videoStatusMap[vid.id] =
              vid.status?.embeddable !== false;
        ...
        console.warn(
          'Could not fetch YouTube video metadata:',
          err
        );
      }
    }
    ...
    const tracks: Track[] = items
      .filter((item: any) => {
        ...
        /*
         * If the videos.list status check returned false,
         * completely remove the video from playable results.
         */
        if (
  ...
  } catch (err: aitle: 'Midnight Resonance',
    artist: 'Aura Sound Lab',
    album: 'Atmospheres Vol. 1',
    duration: 215,
    source: 'stream',
